...
- `query.graph_rag_zero` and `query.soma` are mutually exclusive local retrieval backends. `graph_rag_zero` is embedding-based and enabled by default. `soma` is an embedding-free graph-structural backend available in the default public image. To use Soma, set `query.graph_rag_zero: null` and configure `query.soma: {}` (or a Soma config object). When only embedding-free backends are active (`soma` and/or `code_search`, with `graph_rag_zero: null`), Repo Mind Light skips embedding-API calls during indexing and querying; a CAPI token is still required for chat steps. Switching back to `graph_rag_zero` later requires a full reindex to populate embeddings.
